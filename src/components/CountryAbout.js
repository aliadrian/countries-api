...
const CountryAbout = () => {
  ...
  useEffect(() => {
    const fetchNeighboringCountries = async () => {
      try {
        ...
        const neighboringCountriesData = countriesData.filter((c) => borders.includes(c.cca3));
  ...
  const handleNeighboringCountryClick = async (neighbor) => {
    try {
      const countriesData = await countriesService.getAll();
      const selectedCountry = countriesData.find((c) => c.name.common === neighbor);

      if (selectedCountry) {
        // Redirect to the CountryAbout page for the selected neighboring country
        navigate(`/country/${selectedCountry.name.common}`, { state: { country: selectedCountry } });
      }
    } catch (error) {
  ...
  const { name, capital, region, subregion, population, tld, borders, languages } = country;
            ...
            <div className="country-about-grid">
              <ul className={`grid xl:grid-flow-col gap-4 col-start-1 col-end-3 items-baseline ${calculateRows()}`}>
                ...
                {neighboringCountries.map((neighbor, index) => (
                  <Link to="#" onClick={() => handleNeighboringCountryClick(neighbor)} key={index} className="outline outline-1 outline-lightGray dark:outline-none py-1 dark:text-white dark:bg-darkElement px-3 text-center whitespace-nowrap shadow-sm shadow-darkGray dark:shadow-darkBg rounded" >{neighbor}</Link>
                ))}
              </ul>
            </div>
          )}
