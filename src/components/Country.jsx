...
const Country = ({ country }) => {
  ...
  const alt =
    flagAlt && flagAlt.length !== undefined && flagAlt.length <= 0
      ? `Flag of ${name.common}`
      : flagAlt || `Flag of ${name.common}`;

  return (
    <div className="mx-auto">
      <div className="flex items-center justify-between w-320px w-full"></div>
      <div className="rounded shadow-lg cursor-pointer">
        ...
        <div className="text-left px-6 pt-6 pb-10 dark:text-white dark:bg-darkElement rounded-b-lg">
          <h1 className="font-bold text-lg pb-4 pt-3">
            {name.common.length > 19
              ? `${name.common.substring(0, 19)}...`
              : name.common}
          </h1>
